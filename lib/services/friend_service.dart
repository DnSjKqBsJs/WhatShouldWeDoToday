...
class FriendService {
  ...
  Future<void> sendFriendRequest(String fromId, String toId) async
  {
    ...
    if(user1 != null)
    {
      if(!user1.friends!.contains(toId) && !await requestExit(fromId, toId))
      {
        ...
        final friendRequest = FriendRequestModel(requestId: id ,fromId: fromId, toId: toId);
        await _db.collection('friendsRequest').doc(id).set(friendRequest.toMap());
      }
    }
  }
  ...
  {
    final n1 = await _db.collection('friendsRequest').where('fromId', isEqualTo: id1).where('toId', isEqualTo: id2).get();
    final n2 = await _db.collection('friendsRequest').where('fromId', isEqualTo: id2).where('toId', isEqualTo: id1).get();
  ...
  Future<void> acceptFriendRequest(FriendRequestModel friendRequest) async
  {
    ...
    if(fromIdUser != null && toIdUser != null)
    {
      ...
      batch.update(_db.collection('users').doc(friendRequest.fromId), fromIdUser.toMap());
      batch.update(_db.collection('users').doc(friendRequest.toId), toIdUser.toMap());
      batch.delete(_db.collection('friendsRequest').doc(friendRequest.requestId));
  ...
  {
    await _db.collection('friendsRequest').doc(friendRequest.requestId).delete();
  }
}